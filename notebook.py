...
def build_label_mapping(frame: pd.DataFrame) -> tuple[dict[str, int], pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    # TODO: split the labeled dataframe into train_df, val_df, and test_df. // 80/10/10
    ...
    if n_samples < 5:
        train_df = labelled.iloc[:2]
        val_df = labelled.iloc[2:3]
        test_df = labelled.iloc[3:]
    else:

        n_test_samples = int(n_samples * 0.2)
        min_class_count = labelled['label_id'].value_counts().min()

        can_stratify = (min_class_count >= 2) and (n_test_samples >= 2)
        ...
        train_df, temp_df = train_test_split(
            ...
            test_size = 0.2,
            random_state = 42,
            stratify=labelled['label_id'] if can_stratify else None
        )

        n_samples_temp = len(temp_df)
        n_test_samples_temp = int(n_samples_temp * 0.5)
        min_class_count_temp = temp_df['label_id'].value_counts().min()

        can_stratify_temp = (min_class_count_temp >= 2) and (n_test_samples_temp >= 2)

        val_df, test_df = train_test_split(
            temp_df,
            test_size = 0.5,
            random_state = 42,
            stratify=temp_df['label_id'] if can_stratify_temp else None
        )
